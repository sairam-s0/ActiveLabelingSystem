...
class ModelManager:
    ...
    def compare_models(self, base_path: str, shadow_path: str) -> dict:
        """Compare base and shadow models to decide if promotion is safe."""
        base_file = Path(base_path)
        shadow_file = Path(shadow_path)

        if not base_file.exists():
            return {'recommend_promote': False, 'error': f'Base model not found: {base_path}'}
        if not shadow_file.exists():
            return {'recommend_promote': False, 'error': f'Shadow model not found: {shadow_path}'}

        try:
            base_size = base_file.stat().st_size
            shadow_size = shadow_file.stat().st_size

            # Basic sanity: shadow should not be drastically smaller (corrupted)
            if shadow_size < base_size * 0.1:
                return {
                    'recommend_promote': False,
                    'error': (
                        f'Shadow model is suspiciously small '
                        f'({shadow_size / 1024 / 1024:.1f}MB vs '
                        f'{base_size / 1024 / 1024:.1f}MB base). '
                        f'Possible corruption.'
                    ),
                }

            # Try loading the shadow model to verify it's valid
            if YOLO_AVAILABLE:
                try:
                    test_model = YOLO(shadow_path)
                    shadow_classes = len(getattr(test_model, 'names', {}))
                except Exception as e:
                    return {
                        'recommend_promote': False,
                        'error': f'Shadow model failed to load: {e}',
                    }
            else:
                shadow_classes = 0
            ...
            return {
                ...
                'base_size_mb': round(base_size / 1024 / 1024, 2),
                'shadow_size_mb': round(shadow_size / 1024 / 1024, 2),
                'shadow_classes': shadow_classes,
            }

        except Exception as e:
            return {'recommend_promote': False, 'error': f'Comparison failed: {e}'}
