...
class ShadowTrainer:
    ...
    def _create_yolo_dataset(self, samples: list, root_dir: Path) -> Path:
        ...
        # Shuffle samples to distribute class distributions evenly
        shuffled_samples = list(samples)
        random.shuffle(shuffled_samples)
        
        split_idx = int(len(shuffled_samples) * 0.8)
        if len(shuffled_samples) > 1 and split_idx == len(shuffled_samples):
            split_idx = len(shuffled_samples) - 1
        if split_idx == 0 and len(shuffled_samples) > 0:
            split_idx = 1
        ...
        def write_sample(s, img_dir, lbl_dir) -> bool:
            ...
            # create label
            label_file = lbl_dir / f"{src_img.stem}.txt"
            ...
            with open(label_file, "w") as f:
                for det in s['detections']:
                    ...
                    # convert xyxy to normalized yolo center-x, center-y, width, height
        ...
        for s in train_samples:
            if write_sample(s, train_images_dir, train_labels_dir):
                copied_train += 1
            else:
                skipped_count += 1

        for s in val_samples:
            if write_sample(s, val_images_dir, val_labels_dir):
                copied_val += 1
            else:
                skipped_count += 1

        print(f"[Shadow] Dataset created: {copied_train} train images, {copied_val} val images, {skipped_count} skipped")
        
        # create data.yaml with train/val splits
