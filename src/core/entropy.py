...
class EntropyCalculator:
    ...
    def from_yolo_output(box_output, num_classes: int = 80) -> float:
        # option 0: If box_output itself represents a single box with conf/cls (e.g. boxes[i])
        if hasattr(box_output, 'conf') and hasattr(box_output, 'cls') and not hasattr(box_output, 'boxes'):
            try:
                conf_t = box_output.conf
                cls_t = box_output.cls
                if len(conf_t) > 0:
                    confidence = float(conf_t[0].cpu().item() if hasattr(conf_t, 'cpu') else conf_t[0])
                    pred_class = int(cls_t[0].cpu().item() if hasattr(cls_t, 'cpu') else cls_t[0])
